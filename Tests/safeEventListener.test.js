...
describe('addSafeEventListener', () => {
    ...
    test('should add event listener when el is a valid EventTarget', () => {
      ...
      addSafeEventListener(mockElement, 'click', jest.fn(), { capture: true });
  
      expect(mockListener).toHaveBeenCalledWith('click', expect.any(Function), {
        capture: true,
      });
    });
    ...
    test('should log a warning when el is not a valid EventTarget', () => {
      ...
      addSafeEventListener(invalidElement, 'click', jest.fn(), { capture: true });
